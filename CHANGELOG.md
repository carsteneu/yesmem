...
- Dump request body in OpenAI forward path under YESMEM_PROXY_DEBUG

### Documentation

- Regenerate with 2.3.7, new fixes, version compare links
- Transfer yesloop hardening to research loop
- Strengthen mandate, harden drift/fix rules, sync bundled command

## [2.3.8] - 2026-08-13

### Fixed

- Don't soft-dedup remember() across very different text lengths
- Per-script cap timeout instead of fragile llm() string match
- Don't drop trailing multimodal messages in reverse translation
- Preserve image content parts in both OpenAI<->Anthropic translations

## [2.3.7] - 2026-08-12

### Fixed

- Scrub hardcoded  path from check_test.go
- 1000x RPC speedup via nc -N flag + code_nav false-positive fix

## [2.3.6] - 2026-08-11

### Fixed

- Scope gotcha injection to the hook payload's cwd project
- Handle short status timestamps (#125)
- Use permissionDecision schema for hook-guard output
- Bake graphical session env into systemd unit
- Send absolute project path to daemon
...
- Revalidate paused agents when scratchpad becomes compliant
- Size-based log rotation for daemon and proxy
...
- Scale incremental dedup safely
- Replace hardcoded  path with os.Getwd()
- Terminate GetLearningChain on self-cycles and supersede cycles
- Refuse to sync when target worktree is dirty
- Register scratchpad_append tool
- Emit max_completion_tokens instead of max_tokens for OpenAI upstream
- Relax timing-fragile integration tests for macOS runners
- Relax timing-fragile cache keepalive test for slow runners
- Remove ghost skills that were never installed
- Filter skill suggestions against installed skills
- Cold-review I1+I2 + dead-code cleanup (M4)
- L5 escalation hint + L6 DONE-claim Phase-6 scope
- L4 mark opencode agents unresumable when SessionID empty
- L1 DoneGuard refire state machine + Phase 4 regex relax
- Insert meta text after tool_result blocks, not before
- Log non-200 response bodies for API error diagnosis
- Synthesize missing tool_results for naked tool_use blocks
- Use relative path in integration test
- Suggestion-level post-filter + peekSkillName boundary fix
- Filter stale skill catalog entries at runtime
...
- Reduce CPU load via dead-path filters and wiki cold-tier
- Lazy-load SSE weights, saving 100MB per MCP process
...
- Phase-guard architecture doc + monitoring discipline skill
- Update CHANGELOG + yesloop relay-to-resume timing guide
- Switch star-history to orphan-branch action (v1)
- Add self-hosted star-history chart to README
...
## [2.3.3-github] - 2026-07-15

### Added

- Revalidate paused agents when scratchpad becomes compliant
- MCP tools in llm() subprocess + is_default bugfix
- Use full absolute path as project identifier
- Add opencode.json helper section for agents
- Idle-relay text + done-verify layer + setup-wizard + cross-backend docs
- 6-layer phase-completion guarantee + spawn model resolution + unique agent IDs
- Agent stream-activity tracking
- Add anthropic provider to opencode.json defaults
- Add yesloop Phase 5 REVIEW and scope scratchpad_read
- Yesloop guardrails + worktree check + polling
- Yesloop guardrails + PTY timing fix
- Yesloop — drift detection and convergence gate guardrails
- Add yesloop — autonomous task loop skill
- SYSTEM.md code style and reporting rules + opencode agent backend PTY injection
- Add attribution field, fact category, and flavor ground for narrative quality
- Add backend field to scheduled jobs
- Add opencode agent backend with PTY prompt injection
- Claude Code hook-guard with DeepSeek + RULES.md enforcement
- Inject code-tools-first directive
- Gotcha injection decay + tiered output (top-1 only)
- Integrate pulse learnings into collapse session timeline
- Capture CC away_summary as pulse learnings
- Add --per-commit mode to sync-public.sh
...
- Add BSL 1.1 license (tiered: individual free, business/enterprise paid)

### Changed

- Rewrite 'Types of memories' as proper Markdown
- Final sync for v2.1.17 — CHANGELOG, maintenance, storage (#73)
- Remove yesdocs file from public repo
- Remove yesdocs file from public repo
- Remove SYSTEM.md from public repository
- Remove RULES.md from public repository
- Remove .last-sync-hash from tracking (already in .gitignore)
- Whitelist mode for docs/ in sync-public.sh (63 commits)
- Remove pulse content truncation from timeline
- SEO optimization + landing page content refresh (4 commits)
- Polish landing page — drop caps, terminal install, inline CSS
- Remove old templates and placeholder files

### Fixed

- Relax timing-fragile cache keepalive test for slow runners
- Phase-guard layers — L1 refire, L4 unresumable guard, L5 hint, L6 scope
- Add missing implementation for integration test
- Use relative path in integration test
- Busy_timeout 5000 → 30000 for multi-writer contention
- Code-describe max_tokens 1024 → 8192 for DeepSeek V4 reasoning
- Preserve opencode.json/jsonc during yesmem uninstall
- Opencode free-tier routing + model prefix
- Add Decisions-resolved field to Phase 2 PLAN
- Detection-first + free-tier support + template completeness
- PID-check both freeze paths to prevent false-positive freezes
- Actual SYSTEM.md sync (was blocked by exclude rule)
- Model provenance, metadata-sparse warning, skill improvements
- Harden sync-public.sh + remove hardcoded private paths
- Remove scripts/extract_skills.ts
- Stream-tracking parentSubagentCounts, agent telemetry mapping, resolve-stale
- Agent-telemetry — thread_hash → session_id mapping (#73194)
- Resolve-stale — stale statt resolve (>30d unfinished)
- ResolveProjectDir — check project parameter before _cwd
- Use --window instead of --wait for gnome-terminal — thanks @memyselfandidev (PR #54)
- Opencode TUI spawn — thanks @memyselfandidev (PR #53)
- Load Backend field when restoring scheduled jobs from DB
- Remove redundant ALTER TABLE embedding_vector from hybrid origin test
- Remove .codex from main and restore clean public .gitignore
- API key fallback chain for re-setup
- Worktree HEAD, project key, Code Map display
- CLI client robustness for subscription installs — extractJSON, briefing skip, codex watcher, keepalive budget_tokens, CBM download progress
- Replace LFS pointer with real sse_dyt_512d.bin binary (6KB)
- Align prompt_rewrite test inputs with updated CC target strings
- Truncate pulse content to 150 chars in session timeline
- Set created_at on pulse learnings from JSONL event timestamp
...
- Keepalive interval display uses exact minutes+seconds
- Add missing hookEventName to hook JSON output
- Cache status countdown + usage log threadID
- Remove .last-sync-hash and cache_cost_analysis.py from tracking
- Correct collapsing pipeline description in README
- Keepalive ping + statusline cache display + community docs
- Per-thread cache status, landing page update, dependency cleanup
- Include version in asset filename to match GoReleaser naming
- MacOS CI socket path + README update
- Include internal/config and internal/hooks (rsync exclude was too broad)

### Documentation

- Switch star-history to orphan-branch action (v1)
- Add self-hosted star-history chart to README
- Add MCP tools reference — all 70 tools with parameters and descriptions
- BENCHMARK.md methodology + CCM19 funding + AI agent orientation + Reddit draft
- BENCHMARK.md methodology + CCM19 funding + AI agent orientation
- Remove "PR Workflow" rule 32 from public RULES.md
- BENCHMARK.md methodology hardening + CCM19 funding story
- Reorder sections for clearer narrative flow
- Expand How YesMem Differs table, add Procedural memory differentiator
- Rewrite README from Claude-only to multi-platform
- Add AI agent orientation hints to README and CLAUDE.md
- Convert README 'What You Get' to table format (#32)
- Translate CapFeatures.md and caps-vs-skills-rationale.md to English (#26)
- Update CHANGELOG
- Add auto-generated changelog
- Add pulse/recap feature to Features.md and README.md
- Restore cache keepalive cost analysis lost in overbroad revert
- Add eager tool-result stubbing to Features.md, README, and landing page - Document new proxy pipeline step with stub format tables in Features.md - Add eager stubbing to README context collapsing table - Add feature card to yesmem.io landing page
- Add yesmem.io landing page with GitHub Pages deployment
- Add Windows WSL2 install note

### Testing

- Remove --pure flag expectation from opencode stdin args
- Raise tool definition budget to 21000 chars, count to 60 (64 commits)
...
[2.3.5]: https://github.com/carsteneu/yesmem/compare/v2.3.4...v2.3.5
...
[2.0.0]: https://github.com/carsteneu/yesmem/compare/v1.2.0-beta.1...v2.0.0
[1.2.0-beta.1]: https://github.com/carsteneu/yesmem/compare/v1.2.0...v1.2.0-beta.1
[1.2.0]: https://github.com/carsteneu/yesmem/compare/v1.1.34...v1.2.0
...
[1.1.32]: https://github.com/carsteneu/yesmem/compare/v1.1.31...v1.1.32
[1.1.31]: https://github.com/carsteneu/yesmem/compare/v1.1.30...v1.1.31
[1.1.30]: https://github.com/carsteneu/yesmem/compare/v1.1.29...v1.1.30
[1.1.29]: https://github.com/carsteneu/yesmem/compare/v1.1.28...v1.1.29
