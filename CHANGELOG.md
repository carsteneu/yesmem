...
- Bake graphical session env (DISPLAY, XAUTHORITY, DBUS_SESSION_BUS_ADDRESS) into generated systemd unit so agent terminals spawn visibly under systemd-user
- Send absolute project path to daemon (proxy canonical project path)
- Handle short and empty `LastActive` values in `yesmem status` without panicking
- Migrate `hook-guard` output to `hookSpecificOutput.permissionDecision` schema so Claude Code no longer rejects the response (Closes #123)
- Scope `hook-check` gotcha injection to the hook payload's `cwd` project so foreign-project gotchas no longer leak into unrelated sessions (Closes #124)
